...
// List lists all Teams in the indexer.
func (s *teamLister) List(selector labels.Selector) (ret []*v1alpha.Team, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha.Team))
	})
	return ret, err
}
...
// List lists all Teams in the indexer for a given namespace.
func (s teamNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.Team, err error) {
...
// Get retrieves the Team from the indexer for a given namespace and name.
func (s teamNamespaceLister) Get(name string) (*v1alpha.Team, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha.Resource("team"), name)
	}
	return obj.(*v1alpha.Team), nil
}
