...
func Start(kubernetes kubernetes.Interface, edgenet versioned.Interface) {
	...
	subnamespaceHandler := &Handler{}
	// Create the subnamespace informer which was generated by the code generator to list and watch subnamespace resources
	informer := coreinformer_v1.NewSubNamespaceInformer(
		edgenetClientset,
		metav1.NamespaceAll,
		0,
		cache.Indexers{},
	...
	informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.Infof("Add subnamespace: %s", event.key)
			if err == nil {
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			if reflect.DeepEqual(oldObj.(*corev1alpha.SubNamespace).Status, newObj.(*corev1alpha.SubNamespace).Status) {
				event.key, err = cache.MetaNamespaceKeyFunc(newObj)
				event.function = update
				log.Infof("Update subnamespace: %s", event.key)
				if err == nil {
					queue.Add(event)
				}
			}
		...
		DeleteFunc: func(obj interface{}) {
			...
			log.Infof("Delete subnamespace: %s", event.key)
			if err == nil {
				queue.Add(event)
			}
		},
