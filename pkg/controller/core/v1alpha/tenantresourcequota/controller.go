			...
			if ready == trueStr {
				for _, owner := range nodeObj.GetOwnerReferences() {
					if owner.Kind == "Tenant" {
						TRQCopy, err := edgenetClientset.CoreV1alpha().TenantResourceQuotas().Get(context.TODO(), owner.Name, metav1.GetOptions{})
						if err == nil {
							exists := false
							...
							edgenetClientset.CoreV1alpha().TenantResourceQuotas().Update(context.TODO(), TRQCopy, metav1.UpdateOptions{})
						}
					}
				}
			}
