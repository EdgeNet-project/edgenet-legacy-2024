...
type FederationV1alpha1Interface interface {
	...
	ClustersGetter
	ManagerCachesGetter
	SelectiveDeploymentAnchorsGetter
}

// FederationV1alpha1Client is used to interact with features provided by the federation.edgenet.io group.
type FederationV1alpha1Client struct {
	restClient rest.Interface
}

func (c *FederationV1alpha1Client) Clusters(namespace string) ClusterInterface {
	return newClusters(c, namespace)
}

func (c *FederationV1alpha1Client) ManagerCaches() ManagerCacheInterface {
	return newManagerCaches(c)
}

func (c *FederationV1alpha1Client) SelectiveDeploymentAnchors(namespace string) SelectiveDeploymentAnchorInterface {
	return newSelectiveDeploymentAnchors(c, namespace)
}
...
// NewForConfigAndClient creates a new FederationV1alpha1Client for the given config and http client.
// Note the http client provided takes precedence over the configured transport values.
func NewForConfigAndClient(c *rest.Config, h *http.Client) (*FederationV1alpha1Client, error) {
	config := *c
	if err := setConfigDefaults(&config); err != nil {
		return nil, err
	}
