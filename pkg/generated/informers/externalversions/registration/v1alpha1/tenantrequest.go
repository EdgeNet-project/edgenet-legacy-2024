...
// TenantRequestInformer provides access to a shared informer and lister for
// TenantRequests.
...
func NewTenantRequestInformer(client versioned.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredTenantRequestInformer(client, resyncPeriod, indexers, nil)
}

// NewFilteredTenantRequestInformer constructs a new informer for TenantRequest type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.RegistrationV1alpha1().TenantRequests().List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.RegistrationV1alpha1().TenantRequests().Watch(context.TODO(), options)
			},
		},
		&registrationv1alpha1.TenantRequest{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *tenantRequestInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredTenantRequestInformer(client, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *tenantRequestInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&registrationv1alpha1.TenantRequest{}, f.defaultInformer)
}

func (f *tenantRequestInformer) Lister() v1alpha1.TenantRequestLister {
	return v1alpha1.NewTenantRequestLister(f.Informer().GetIndexer())
}
