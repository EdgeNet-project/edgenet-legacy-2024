...
type SelectiveDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.SelectiveDeployment, err error)
...
// List lists all SelectiveDeployments in the indexer.
func (s *selectiveDeploymentLister) List(selector labels.Selector) (ret []*v1alpha1.SelectiveDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.SelectiveDeployment))
	})
	return ret, err
}
...
type SelectiveDeploymentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.SelectiveDeployment, error)
...
// List lists all SelectiveDeployments in the indexer for a given namespace.
func (s selectiveDeploymentNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.SelectiveDeployment, err error) {
...
// Get retrieves the SelectiveDeployment from the indexer for a given namespace and name.
func (s selectiveDeploymentNamespaceLister) Get(name string) (*v1alpha1.SelectiveDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("selectivedeployment"), name)
	}
	return obj.(*v1alpha1.SelectiveDeployment), nil
}
