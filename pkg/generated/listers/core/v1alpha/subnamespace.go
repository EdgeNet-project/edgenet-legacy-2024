...
type SubNamespaceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha.SubNamespace, err error)
...
// List lists all SubNamespaces in the indexer.
func (s *subNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.SubNamespace, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha.SubNamespace))
	})
	return ret, err
}
...
type SubNamespaceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha.SubNamespace, error)
...
// List lists all SubNamespaces in the indexer for a given namespace.
func (s subNamespaceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.SubNamespace, err error) {
...
// Get retrieves the SubNamespace from the indexer for a given namespace and name.
func (s subNamespaceNamespaceLister) Get(name string) (*v1alpha.SubNamespace, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha.Resource("subnamespace"), name)
	}
	return obj.(*v1alpha.SubNamespace), nil
}
